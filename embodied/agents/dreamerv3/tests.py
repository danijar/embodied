...
class AgentTest(absltest.TestCase):
  ...
  def test_train_simple(self):
    ...
    config = config.update({'encoder.cnn': 'simple', 'decoder.cnn': 'simple'})
    ...
    assert times[0] <= SLACK * 340.0, times   # TODO: Adjust expectations.
    assert min(times) <= SLACK * 0.05, times  # TODO: Adjust expectations.

  def test_train_resnet(self):
    from . import agent as agnt
    config = embodied.Config(agnt.Agent.configs['defaults'])
    config = config.update(TEST_CONFIG)
    config = config.update({'encoder.cnn': 'resnet', 'decoder.cnn': 'resnet'})
    step = embodied.Counter()
    env = embodied.envs.load_env(config.task, **config.env)
    agent = agnt.Agent(env.obs_space, env.act_space, step, config)
    data = self._make_data(env, [config.batch_size, config.replay_chunk])
    ...
    for _ in range(5):
      ...
      times.append(time.time() - start)
    print('TRAIN DURATIONS:', times)
    assert times[0] <= SLACK * 340.0, times
    assert min(times) <= SLACK * 0.05, times
  ...
  #   assert min(times) <= SLACK * 0.002, times
