...
class Random(tfutils.Module):
  ...
  def actor(self, feat):
    ...
    if self.config.actor.dist == 'onehot':
      return tfutils.OneHotDist(tf.zeros(shape))
    else:
...
class Plan2Explore(tfutils.Module):
  ...
  def __init__(self, config, act_space, wm, reward):
    ...
    self._networks = [
        nets.MLP(size, **config.expl_head)
        for _ in range(config.disag_models)]
    self.opt = tfutils.Optimizer('expl', **config.expl_opt)
  ...
  def _train_ensemble(self, inputs, targets):
    ...
    targets = tf.stop_gradient(targets.astype(tf.float32))
...
class ModelLoss(tfutils.Module):
  ...
  def __init__(self, config, act_space, wm, reward):
    ...
    self.actor = self.ac.actor
    self.head = nets.MLP([], **self.config.expl_head)
    self.opt = tfutils.Optimizer('expl', **self.config.expl_opt)
