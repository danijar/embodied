...
def train_hiro(
  ...
  def train_add(tran, worker):
    tran = tran.copy()
    del tran['image']
    train_replay.add(tran, worker)

  def eval_add(tran, worker):
    tran = tran.copy()
    del tran['image']
    eval_replay.add(tran, worker)

  driver_train = embodied.Driver(train_env)
  driver_train.on_episode(lambda ep, worker: per_episode(ep, mode='train'))
  driver_train.on_step(lambda tran, _: step.increment())
  driver_train.on_step(train_add)
  driver_eval = embodied.Driver(eval_env)
  driver_eval.on_step(eval_add)
  ...
  def train_policy(obs, state=None):
    obs = obs.copy()
    del obs['image']
    mode = 'explore' if should_expl(step) else 'train'
    return agent.policy(obs, state, mode)

  def eval_policy(obs, state=None):
    obs = obs.copy()
    del obs['image']
  ...
  while step < args.steps:
    logger.write()
    driver_eval.reset()
    driver_eval(eval_policy, episodes=max(len(eval_env), args.eval_eps))
    driver_train(train_policy, steps=args.eval_every)
    checkpoint.save()
