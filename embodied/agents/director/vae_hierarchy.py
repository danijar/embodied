...
class VAEHierarchy(tfutils.Module):
  ...
  def __init__(self, wm, act_space, config):
    ...
    if config.explorer:
      self.explorer = agent.ImagActorCritic({
          'expl': agent.VFunction(self.elbo_reward, config),
  ...
  def policy(self, state, carry):
    ...
    new_goal = self.cast(self.dec(
        {'skill': skill, 'context': context}).mode())
    skill = switch(skill, new_skill)
    goal = switch(goal, new_goal)
    dist = self.worker.actor(
        {**state, 'skill': skill, 'context': context, 'goal': goal})
  ...
  def train_jointly(self, imagine, start, data):
    ...
    with tf.GradientTape(persistent=True) as tape:
      ...
      traj['reward_expl'] = self.elbo_reward(traj)
  ...
  def train_vae_imag(self, traj):
    metrics = {}
    context = self.feat({k: v[0] for k, v in traj.items()}).astype(tf.float32)
    goal = self.feat({k: v[-1] for k, v in traj.items()}).astype(tf.float32)
    with tf.GradientTape() as tape:
      enc = self.enc({'goal': goal, 'context': context})
      dec = self.dec({'skill': enc.sample(), 'context': context})
      rec = -dec.log_prob(tf.stop_gradient(goal.astype(tf.float32)))
  ...
  def report(self, data):
    metrics = {}
    for skills in ('manager', 'prior', 'replay'):
  ...
  def report_skills(self, data, skills):
    ...
    if skills == 'manager':
      ...
      target = tf.random.shuffle(self.feat(start))
      skill = self.enc({'goal': target, 'context': self.feat(start)}).sample()
    context = self.feat(start)
    goal = self.dec({'skill': skill, 'context': context}).mode()
