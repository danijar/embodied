...
class DistLayer(tfutils.Module):
  ...
  def inner(self, inputs):
    ...
    if self._dist == 'normal':
      ...
      dist.prior = tfd.Independent(
          tfd.Normal(tf.zeros(self._shape), hi * tf.ones(self._shape)),
          len(self._shape))
      return dist
    ...
    if self._dist == 'trunc_normal':
      ...
      dist.minent = np.prod(self._shape) * tfd.TruncatedNormal(0.99, lo).entropy()
      dist.maxent = np.prod(self._shape) * tfd.TruncatedNormal(0.0, hi).entropy()
      dist.prior = tfd.Independent(tfd.TruncatedNormal(
          tf.zeros(self._shape), hi * tf.ones(self._shape)), 1)
      return dist
    if self._dist == 'onehot':
      ...
      dist.prior = tfutils.OneHotDist(0 * out)
      if len(self._shape) > 1:
        dist.prior = tfd.Independent(dist.prior, len(self._shape) - 1)
      return dist
