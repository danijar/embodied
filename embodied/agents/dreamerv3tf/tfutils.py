...
class Optimizer(Module):
  ...
  def __call__(self, tape, loss, modules):
    ...
    if ~overflow:
      ...
      elif self._opt == 'adalim':
        self._apply_adalim(params, grads)
      else:
        raise NotImplementedError(self._opt)
      # TODO
  ...
  def _apply_adalim(self, params, grads, beta1=0.9, beta2=0.999, lim=1e-5):
    lr = self._lr() if callable(self._lr) else self._lr
    t = self._step.astype(tf.float32)
    for param, grad in zip(params, grads):
      assert not isinstance(grad, tf.IndexedSlices), type(grad)
      ...
      m_hat = self._m[name] / (1. - beta1 ** t)
      v_hat = self._v[name] / (1. - beta2 ** t)
      scale = tf.math.rsqrt(tf.maximum(v_hat, self._eps ** 2))
      param.assign_sub(lr * m_hat * scale)
