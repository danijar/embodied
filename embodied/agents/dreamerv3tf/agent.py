...
class ImagActorCritic(tfutils.Module):
  ...
  def __init__(self, critics, scales, act_space, config):
    ...
    self.actor = nets.MLP(act_space.shape, **self.config.actor, dist=dist)
    if self.config.slow_actor:
      self.updates = tf.Variable(-1, dtype=tf.int64)
      self.slow_actor = nets.MLP(
          act_space.shape, **self.config.actor, dist=dist)
    self.grad = (
  ...
  def loss(self, traj, score):
    ...
    if self.config.slow_actor:
      pi = policy
      pi_slow = self.slow_actor(tf.nest.map_structure(tf.stop_gradient, traj))
      pi_rand = policy.prior
      if len(self.actent.shape) > 0:
        assert isinstance(pi, tfd.Independent)
        assert isinstance(pi_slow, tfd.Independent)
        assert isinstance(pi_rand, tfd.Independent)
        pi = pi.distribution
        pi_slow = pi_slow.distribution
        pi_rand = pi_rand.distribution
      if self.config.policy_prior == 'ema':
        slow_kl = tfd.kl_divergence(pi, pi_slow)
        slowkl_loss, mets = self.slowkl(slow_kl)
        metrics.update({f'slowkl_{k}': v for k, v in mets.items()})
        loss += slowkl_loss.mean()
      elif self.config.policy_prior == 'rand':
        slow_kl = tfd.kl_divergence(pi, pi_rand)
        slowkl_loss, mets = self.slowkl(slow_kl)
        metrics.update({f'slowkl_{k}': v for k, v in mets.items()})
        loss += slowkl_loss.mean()
      elif self.config.policy_prior == 'mix':
        m = self.config.policy_prior_mix
        slow_kl = (
            m * tfd.kl_divergence(pi, pi_rand) +
            (1 - m) * tfd.kl_divergence(pi, pi_slow))
        slowkl_loss, mets = self.slowkl(slow_kl)
        metrics.update({f'slowkl_{k}': v for k, v in mets.items()})
        loss += slowkl_loss.mean()
      else:
        raise NotImplementedError(self.config.policy_prior)
    loss *= tf.stop_gradient(traj['weight'])[:-1]
    return loss, metrics

  def update_slow(self):
    if not self.config.slow_actor:
      return
    assert self.actor.variables
    initialize = (self.updates == -1)
    mix = 1.0 if initialize else self.config.slow_actor_fraction
    for s, d in zip(self.actor.variables, self.slow_actor.variables):
